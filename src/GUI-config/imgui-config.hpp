...
//* vulkan setup, instance, extensions
static void SetupVulkan(ImVector<const char*> instance_extensions)
{
    ...
    if (deviceCount > 0)
    {
        VkPhysicalDevice devices[deviceCount];  // Use a fixed-size array
        vkEnumeratePhysicalDevices(g_Instance, &deviceCount, devices);
        physicalDevice = devices[0]; // Select the first available device
    }
