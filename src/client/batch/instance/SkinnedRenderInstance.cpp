#include "SkinnedRenderInstance.hpp"
...
void SkinnedRenderInstance::addRegistration(
...
void SkinnedRenderInstance::updateModelMatrix()
{
    glm::mat4 model(1.0f);
    ...
    {
        auto& data = reg.batch->getinstancesData()[reg.indexInBatch];

        data.model = model;
    }
}

void SkinnedRenderInstance::updateAnimation(float dt)
{
    if (animator)
        animator->update(dt);
}
