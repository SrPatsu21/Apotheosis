...
VertexManager::VertexManager(const std::vector<Vertex> vertices)
{
    ...
    allocateVertexBufferMemory(bufferSize);
...
void VertexManager::createVertexBuffer(VkDeviceSize bufferSize){
...
void VertexManager::allocateVertexBufferMemory(VkDeviceSize bufferSize){
    ...
    allocInfo.memoryTypeIndex = CoreVulkan::findMemoryType(memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
